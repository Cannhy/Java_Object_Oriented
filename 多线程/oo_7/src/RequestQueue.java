...
public class RequestQueue {
    ...
    public boolean useful(Elevator temp) {
        int cnt = 0;
        for (Integer tempk : requestMap.keySet()) {
            if (temp.arriveAble(tempk)) {
                for (Person temp2 : requestMap.get(tempk)) {
                    if (temp.arriveAble(temp2.getDes())) {
                        cnt++;
                    }
                }
            }
